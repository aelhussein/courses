...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self, text):
        ...
        text_lower_split = re.split("[" + separator + "]+", text_lower)
        text_join  = " ".join(text_lower_split) + ' '
        cleaned_phrase = self.phrase.lower() + ' ' 
        if cleaned_phrase in text_join:
            return True
        return False
...
def read_trigger_config(filename):
    ...
    for line in lines:
        ...
        if split[0] != 'ADD':
            if split[1] == 'TITLE':
                prelim_triggers[split[0]] = TitleTrigger(split[2])
            elif split[1] == 'DESCRIPTION':
                prelim_triggers[split[0]] = DescriptionTrigger(split[2])
            elif split[1] == 'BEFORE':
                prelim_triggers[split[0]] = BeforeTrigger(split[2])
            elif split[1] == 'AFTER':
                prelim_triggers[split[0]] = AfterTrigger(split[2])
            elif split[1] == 'NOT':
                prelim_triggers[split[0]] = NotTrigger(split[2])
            elif split[1] == 'OR':
                prelim_triggers[split[0]] = OrTrigger(split[2],split[3])
            else:
                prelim_triggers[split[0]] = AndTrigger(split[2],split[3])
        else:
            for s in split:
                    if s in prelim_triggers.keys():
                        triggers[s] = prelim_triggers[s]
    return triggers

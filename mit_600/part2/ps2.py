    ...
    for line in lines:
        for i in range(0,2):
            ...
            if not graph.has_node(node):
                graph.add_node(node)
        graph.add_edge(WeightedEdge(Node(line[0]), Node(line[1]), int(line[2]), int(line[3])))
    return graph
...
#
# Answer: To arrive in the shortest distance without exceeding time outdoors
#
...
def find_distance(digraph, path):
    total_dist= 0
    outdoor_dist = 0
    ...
    if start not in digraph.nodes and end not in digraph.nodes :
        ...
        for node in digraph.get_edges_for_node(start):
            ...
            if dest not in path:
                ...
                if newPath != None:
                    total_dist, outdoor_dist= find_distance(digraph, newPath)

...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles = {}
        if self.width <= 0 or self.height <= 0 or self.dirt_amount <= 0:
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        dirt_left = self.tiles[(tile_x, tile_y)]
        if (tile_x, tile_y) not in self.tiles:
            raise ValueError
        else:
            if  dirt_left >= self.capacity:
                self.tiles[(tile_x, tile_y)] -= self.capacity
            else:
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        if self.speed <= 0 or self.capacity <= 0:
            raise ValueError
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        x = math.floor(pos.get_x())
        y = math.floor(pos.get_y())
        if (x,y) in self.tiles and (x,y) not in self.furniture_tiles:
            return True
    ...
    def get_num_tiles(self):
        ...
        w = self.width
        h = self.height
        return w*h - len(self.furniture_tiles)
    ...
    def get_random_position(self):
        ...
        while x < 1: 
            x = random.random()*self.width
            y = random.random()*self.height
            if (math.floor(x),math.floor(y)) in self.tiles and (math.floor(x), math.floor(y)) not in self.furniture_tiles:
                x = 1
        self.position = Position(x,y)
        return self.position
...
#room_spec = [(10,30),(20,15),(25,12),(50,6)]
#for spec in room_spec:
#    print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, spec[0], spec[1], 3, 0.8, 50, StandardRobot)))
#    print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, spec[0], spec[1], 3, 0.8, 50, FaultyRobot)))

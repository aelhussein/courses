...
class Linear(Module):
    ...
    def backward(self, dLdZ):  # dLdZ is (n x b), uses stored self.A
        ...
        self.dLdA = np.dot(self.W,dLdZ)        # Your code: return dLdA (m x b)
        return self.dLdA

    def sgd_step(self, lrate):  # Gradient descent step
        self.W  = self.W - lrate*(self.dLdW)  # Your code
        self.W0 = self.W0 - lrate*np.sum(self.dLdW0, axis=1, keepdims=True)  # Your code
...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        self.dLdZ = np.dot((1-self.A**2).T,dLdA)
        return   self.dLdZ# Your code: return dLdZ (b, b)


class ReLU(Module):  # Layer activation
    def forward(self, Z):
        self.A = np.where(Z<0,0,Z)  # Your code: (m, b)
        return self.A

    def backward(self, dLdA):  # uses stored self.A
        self.dLdZ = np.dot(np.where(self.A>0,1, 0).T,dLdA)
        return self.dLdZ  # Your code: return dLdZ (b, b)


class SoftMax(Module):  # Output activation
    def forward(self, Z):
         return np.exp(Z)/np.sum(np.exp(Z))  # Your code: (m, b)
...
class NLL(Module):  # Loss
    def forward(self, Ypred, Y):
        self.Ypred = Ypred
        self.Y = Y
        return -np.sum(self.Y*np.log(self.Ypred))  # Your code: return loss (scalar)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            ...
            Xt , Yt = X[:,num:num+1], Y[:,num:num+1]
            Yt_pred = self.forward(Xt)
            loss += self.loss.forward(Yt_pred, Yt)

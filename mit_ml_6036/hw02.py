...
th = np.zeros(len(data1[:,0]))
th.shape
labels1[:,0].shape
labels1[:,0]*np.dot(th.T, data1[:,0])
...
def perceptron(data, labels, params={}, hook=None):
    ...
    for j in range(T):
        for i in range(len(data[0])):
            if labels[:,i]*(np.dot(theta.T, data[:,i]) + theta_0) <= 0:
                theta += labels[:,i]*data[:,i]
                theta_0 += labels[:,i]
    theta = np.reshape(theta,(len(theta),1))
    theta_0 = np.reshape(theta_0, (1,1))
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    theta_0s = np.reshape(theta_0s, (1,1))
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ...
    correct = score(data_test, labels_test, theta, theta_0)
    total = len(labels_test[0])
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        x_test, y_test = data_gen(n_test)
        scores += eval_classifier(learner, x_train, y_train, x_test, y_test)
    return scores/it
...
def xval_learning_alg(learner, data, labels, k):
    ...
    for i in range(k):
        x_train = split_data.copy()
        del x_train[i]
        y_train = split_labels.copy()
        del y_train[i]
...
avg = 0
for i in range(100):
    avg += (eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5))
avg/100


avg = 0
for i in range(100):
    avg += (eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5))
avg/100

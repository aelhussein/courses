...
def sgd_step(data, x, lam, step):
    ...
    # Your code here
    temp_a = u[a]
    e = u[a].T@v[i] + b_u[a] + b_v[i] - r
    u[a] = u[a] - step*(e*v[i] + lam_u[a]*u[a])
    b_u[a] = b_u[a]- step*(e)
    v[i] = v[i] - step*(temp_a@(e) + lam_v[i]*v[i])
    b_v[i] = b_v[i]- step*(e)
    x = (u, b_u, v, b_v)
    return x
...
for i in movies_dict:
    if i  == 260: continue
    sim[i] = float(v[260].T@v[i]/(np.linalg.norm(v[260]*np.linalg.norm(v[i]))))
movies = sorted(sim, key=lambda k: sim[k])[-10:]
for mov in movies:
    print(movies_dict[mov])    

sim= {}
for i in movies_dict:
    if i  == 2628: continue
    sim[i] = float(v[2628].T@v[i]/(np.linalg.norm(v[2628]*np.linalg.norm(v[i]))))
sorted(sim, key=lambda k: sim[k])[-10:]

sim= {}
for i in movies_dict:
    for j in movies_dict:
        if i == j: continue
        sim[(i,j)] = float(v[i].T@v[j]/(np.linalg.norm(v[i]*np.linalg.norm(v[j]))))

sum(sim.values())/len(sim)


genres_2 = {genre: [] for genre in genres}
for mov in genres_dict:
    for genre in genres_dict[mov]:
        genres_2[genre].append(mov)
        
gen_sim = {genre: [] for genre in genres}
for movies in genres_2:
    sim = []
    for i in genres_2[movies]:
        for j in genres_2[movies]:
            if i == j: continue
            sim.append(float(v[i].T@v[j]/(np.linalg.norm(v[i]*np.linalg.norm(v[j])))))
    gen_sim[movies] = np.mean(sim)
            
for movies in genres_2:
    sim = []
    for i in genres_2['Comedy']:
        for j in genres_2[movies]:
            if i == j: continue
            sim.append(float(v[i].T@v[j]/(np.linalg.norm(v[i]*np.linalg.norm(v[j])))))
    gen_sim[movies] = np.mean(sim)
